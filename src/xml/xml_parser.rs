...
fn processing_router(line: String, tree: &mut XMLTree) {
    ...
    if trim_line.is_empty() {
        ...
        handle_exit_tag(trim_line.to_string(), tree); // handle the exit tag
    } else if trim_line.starts_with('<') {
        handle_entry_tag(trim_line.to_string(), tree); // handle the entry tag
    } else if TEXT.is_match(trim_line) {
        handle_characters(trim_line.to_string(), tree); // handle the text
    }
}
